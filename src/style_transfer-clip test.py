...
def style_diffusion_fine_tuning(
    ...
    # if logger is not None:
    #     logger.info(f"Checking CLIP preprocess transforms:")
    #     for idx, t in enumerate(clip_preprocess.transforms):
    #         logger.info(f"  [{idx}] {t}")
    ...
    for iter in range(k):
        ...
        for i in range(len(content_latent)):
            ...
            for step in range(len(ddim_timesteps_backward)-1):
                ...
                manual_norm = T.Normalize(mean=[-1.0, -1.0, -1.0], std=[2.0, 2.0, 2.0])
                img_0_1 = manual_norm(x_t_prev)

                # === [调试代码开始] 保存一张看看这一步对不对 ===
                if iter == 0 and i == 0 and step == 0:
                    # [修改点] 不要用 CONTENT_GRAY_PATH，直接存到 output 文件夹下
                    save_debug_path = "output/debug_clip_input.jpg"
                    
                    # 确保 output 文件夹存在
                    os.makedirs("output", exist_ok=True)

                    # 简单的保存 tensor 为图片
                    debug_img = img_0_1[0].detach().cpu() # 取 batch 第一张
                    T.ToPILImage()(debug_img).save(save_debug_path)
                    
                    # 使用 logger 打印，确保能看到日志
                    if logger is not None:
                        logger.info(f"Debug image saved to {save_debug_path}. Please check it!")
                    else:
                        print(f"Debug image saved to {save_debug_path}. Please check it!")
                # === [调试代码结束] ===

                # 2. 组合剩下的 CLIP 预处理
                # 注意：这里的 transforms 必须对应 Resize/Crop 和 ImageNet Normalize
                clip_transform_layers = clip_preprocess.transforms[:2] + clip_preprocess.transforms[4:]
                clip_layer = T.Compose(clip_transform_layers)
                
                # 3. 最终输入 CLIP 的 tensor
                clip_input = clip_layer(img_0_1)

                I_content_color = clip_model.encode_image(clip_input)
